 ...
 *
 * <p>BR8.6は「行番号・フィールド単位のエラーメッセージ」を要求する。entities.mdの{@code
 * CsvImportRowResult.errors}は{@code {field, message}}の形状を定める一方、C13契約
 * (contract-summary.md)の{@code DataImportExportApi.importCsv}が返す{@code ImportResult.errors:
 * List<RowError>}は{@code RowError { row: int, message: string }}と定める。本レコードは両者を
 * 満たす上位互換の形状({@code row}・{@code field}・{@code message}の3項目)として1つに統合する
 * (contract-summary.mdの契約所有ルール「加法的変更(オプションフィールド追加等)は、コンシューマーが
 * 未知のフィールドを無視することを前提に、プロバイダー単独の判断で行ってよい」に基づく、フィールド単位の
 * 詳細情報を保持するための追加)。
 *
