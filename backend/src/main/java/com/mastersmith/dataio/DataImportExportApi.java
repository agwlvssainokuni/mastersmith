...
/**
 * data-import-export(U8)が提供する内部Javaインタフェース契約 (inception/contract-design/contract-summary.md C13:
 * DataImportExportApi)。
 *
 * <p>consumers: list-engine(exportCsv)、record-edit-engine(importCsv)。DataImportExportは
 * config-engine(U1)のみに依存し、権限は呼び出し元が事前検証済みであることを前提に再検証しない (functional-design/rules.md BR8.8)。
 *
 * <p><b>C13契約からのexportCsvシグネチャ上の差異</b>: contract-summary.mdはexportCsvの戻り値を{@code InputStream
 * (CSV)}と記す。しかしnfr-design/performance-design.mdは、BR8.10(ストリーミング処理、全件を
 * メモリへ読み込まない)を実現するため、業務データ用RDBMSのカーソルから読み取った行を都度呼び出し元の {@code OutputStream}(例: {@code
 * HttpServletResponse}の出力ストリーム)へ直接書き込む方式を明示的に選定した
 * (「HttpServletResponseの出力ストリームへ直接ストリーミングする」)。本インタフェースはこの性能設計を 具体化し、CSVデータを保持する新規{@code
 * InputStream}を返す代わりに、呼び出し元が渡す{@code OutputStream}
 * へ書き込む方式を採用する(戻り値でCSVバイト列を返す設計は、全件をメモリ上に保持するか別スレッドで
 * パイプ処理するかのいずれかを要求し、BR8.10の制約と両立しないため)。この差異はC13契約の加法的な実装詳細 であり、業務ロジック(BR8.1〜BR8.10)そのものには影響しない。
 */
