...
public class SessionService {
  ...
  public int rotate(
    ...
    return readRoleId == null
        ? repository.rotateWithoutRole(
            sessionId, oldHash, newHash, now, newRefreshExpiresAt, newRoleId)
  ...
  /**
   * リフレッシュ時のアクティブロールの再確認(BR5.10、security-design.md NFR2.6の判定表)。現在のロール{@code
   * current}が選択済みで、最新のロール集合に 含まれるなら、そのまま。それ以外は、ロール集合がちょうど1つなら、そのロール(自動選択)、0個・2個以上なら未選択(null)。
   */
