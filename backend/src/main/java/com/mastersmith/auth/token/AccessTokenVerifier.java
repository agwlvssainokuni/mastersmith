...
/**
 * アクセストークン(JWT)を検証する(NFR2.2)。Nimbus JOSE + JWTを直接使い、Spring SecurityのOAuth2 Resource Serverの{@code JwtDecoder}は
 * 使わない(署名方式のHS256限定・時計のずれ0・すべての失敗を同一の401にする、を自前で確実に制御するため)。
 *
 * <p>手順(いずれかに失敗した時点で{@link InvalidAccessTokenException}): (1)JWSのコンパクト形式として解析できる。(2)ヘッダーの{@code alg}が
 * {@code HS256}である({@code none}・HS384・HS512・RS256などは、署名の検証に進まず拒否)。(3)HMAC-SHA256の署名が、注入された鍵で検証できる。
 * (4){@code sub}・{@code sid}・{@code iat}・{@code exp}が存在し、妥当である。(5){@code exp}が、注入された{@link Clock}の現在時刻より後である
...
public class AccessTokenVerifier {
  ...
  public AccessTokenClaims verify(String token) {
    ...
    if (sub == null || sub.isEmpty() || sid == null || sid.isEmpty() || iat == null || exp == null) {
      throw invalid();
    }
