...
/**
 * 認証のエラー応答の種類(security-design.md NFR2.8)。ステータス・{@code title}・{@code detail}(状況ごとの固定の文言。原因の詳細・入力値を含めない)・
 * {@code code}(安定したi18nキー。frontend-ui(U12)が、キーから、表示する文言を選ぶ、NFR7.1)・{@code WWW-Authenticate: Bearer}を付けるかを、1か所に持つ。
 * フィルタでの応答({@link ProblemDetailsWriter})と、コントローラでの応答({@code AuthApiExceptionAdvice}・{@code
 * AuthCrossCuttingExceptionAdvice})が、同じ内容を返す。
 */
public enum AuthProblem {
  ...
  /** 保持していないロールの選択(BR5.9)。 */
  ROLE_NOT_HELD(
      HttpStatus.FORBIDDEN, "Forbidden", "選択できないロールです。", "auth.role.not-held", false),
  /** 認証の要否の規則による拒否(deny by default)。 */
  FORBIDDEN(
      HttpStatus.FORBIDDEN, "Forbidden", "この操作を実行する権限がありません。", "auth.forbidden", false),
