 ...
 * targetTableConfigId == null})自体への個別の権限判定は行わず、配下(子孫)の可視性から再帰的に導出する(BR6.4:
 * 配下に1件でも表示可能な項目があれば表示、全て非表示なら除外)。各階層は{@code order}昇順でソートする(BR6.6、重複時は安定ソートに委ねる)。
 *
 * <p>管理メニュー4項目({@link AdminMenuDefinition})は、対応する予約screenKeyで{@code
 * canAccessScreen}を呼び出しtrueの項目のみを返す(BR6.2・BR6.3-(3)〜(5))。
 */
@Component
public class MenuTreeBuilder {
  ...
  /**
   * businessMenu(BR6.1)を組み立てる。{@code allItems}は{@link
   * com.mastersmith.menu.repository.MenuItemRepository#findAll()}の結果を渡す。
   */
  public List<MenuItemView> buildBusinessMenu(List<MenuItem> allItems, String activeRoleId) {
    ...
    for (MenuItem item : allItems) {
      childrenByParent
          .computeIfAbsent(item.getParentMenuItemId(), key -> new ArrayList<>())
          .add(item);
    }
  ...
  private Optional<MenuItemView> buildVisibleNode(
    ...
    List<MenuItemView> visibleChildren =
        buildVisibleSiblings(children, childrenByParent, activeRoleId);
    if (visibleChildren.isEmpty()) {
      // BR6.4: 配下が全て非表示(子を持たない場合を含む)ならフォルダ自体も非表示。
      return Optional.empty();
    }
    return Optional.of(
        new MenuItemView(item.getMenuItemId(), item.getLabel(), null, visibleChildren));
  }
  ...
  private Optional<MenuItemView> buildVisibleLeaf(MenuItem item, String activeRoleId) {
    ...
    return Optional.of(
        new MenuItemView(
            item.getMenuItemId(), item.getLabel(), item.getTargetTableConfigId(), List.of()));
  }
