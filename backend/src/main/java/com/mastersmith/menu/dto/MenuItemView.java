 ...
 *
 * <p>{@code GET /api/menu}のbusinessMenu/adminMenu配列の各ノード(children込みの木構造)と、{@code POST/PUT
 * /api/menu-items}の作成・更新結果(childrenは空配列)の両方に用いる。C3契約は{@code order}をレスポンスに含めないため
 * (作成・更新時の入力にのみ現れる)、本レコードにも{@code order}は含めない。
 *
 * @param menuItemId メニュー項目のID(管理メニューの場合は{@link com.mastersmith.menu.tree.AdminMenuDefinition}の固定ID)
 * @param label 表示名
