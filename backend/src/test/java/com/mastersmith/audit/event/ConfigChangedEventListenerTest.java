...
/**
 * {@link
 * ConfigChangedEventListener}の単体テスト。正常系(リポジトリ保存)と、リポジトリが例外を送出しても発行元へ再送出しないこと(NFR4.2・NFR4.5、rules.md
 * BR7.7)を検証する。
 */
class ConfigChangedEventListenerTest {
  ...
  private final ConfigChangedEventListener listener =
      new ConfigChangedEventListener(mapper, repository);

  private static ConfigChangedEvent anEvent() {
    return new ConfigChangedEvent(
        ConfigChangeOperation.DRAFT_IMPORTED, "shop.products", "system", Instant.now());
  }

  @Test
  void savesAMappedAuditLogEntryWhenTheRepositorySucceeds() {
    listener.onConfigChangedEvent(anEvent());
  ...
  void doesNotPropagateAnExceptionWhenTheRepositoryFails() {
    ...
    // BR7.7: 例外は構造化ログへ記録するのみとし、発行元(config-engine)へは再送出しない。
    assertThatCode(() -> listener.onConfigChangedEvent(anEvent())).doesNotThrowAnyException();
  }
}
