...
rules:
  ...
  - id: BR5.3
    statement: >
      ...
      `consecutiveFailures`を0に、`lockedUntil`を空に戻したうえで、1増やす。(3)それ以外で、`consecutiveFailures`が
      しきい値(既定5回)未満なら、1増やす。(4)`consecutiveFailures`がしきい値以上で`lockedUntil`が空なら(しきい値に達した
      試行が、まだ検証の途中)、確保しない。確保できなかった場合は、検証せず(BR5.2の`dummyVerify`を行って)失敗の応答を返し、
      失敗回数を数えず、ロック期間も延長しない。確保できた場合は、C11で検証する。検証が成功したら、`consecutiveFailures`を
      0に戻し、`lockedUntil`を空にする。失敗したら、その枠を失敗として確定し、`consecutiveFailures`がしきい値に達して
      いれば、`lockedUntil`を、現在時刻に設定値(既定15分)を加えた日時にする(この更新は条件付きで、他の試行の結果を上書き
      しない)。ハッシュ計算の上限超過(503、BR5.15)や、検証の途中の中断の場合は、確保した枠を返す(1戻す。中断で戻せ
      なかった場合は、失敗として数えたままにする=安全側)。この方式により、同時の試行が何件あっても、1回のロック期間
      (またはロックの解除から次のロックまで)に検証できる試行は、しきい値の回数を超えない。登録されていないメール
      アドレス・招待中・無効化済みのユーザーへの試行は、失敗回数を記録しない(存在の有無を外部に漏らさないため、また、
      それらは成功しえないため)。ロックは、`lockedUntil`の経過で自動的に解除される。予約の更新・結果の更新は、
      それぞれ短いトランザクションで行い、C11の呼び出しの間は、トランザクションを保持しない(資源の取得順序の不変条件)。
    category: policy
    applies_to: [AccountLoginState]
    trigger: "activeなユーザーへのログイン試行時"
    logic: "IF 枠を確保できない THEN 検証せずBR5.2の失敗応答。ELSE 検証する: 成功なら回数を0に戻す。失敗なら枠を失敗として確定し、しきい値に達していればlockedUntilを設定する(応答はBR5.2の失敗応答)。上限超過なら枠を返して503。"
    violation_behaviour: "401 Unauthorized(BR5.2)、503(BR5.15)"
    source: "FR2.7, functional-design-questions.md Q2, レビュー指摘R-04"
  ...
  - id: BR5.15
    statement: >
      ...
      この場合は、失敗回数に数えず、確保した枠を返す(BR5.3)。C11のメソッド(`findByEmail`・`findByUserId`・
...
| BR5.3 | policy | アカウントロック(予約型の原子的な更新、5回・15分、成功でリセット、ロック中は数えず延長しない、自動解除) |
