...
- **計測の方法と合否**(NFR1.1の検証): team.mdが負荷・性能テストを既定に含めないため、統合テストで、次の条件で計測して、結果をBuild and Testの結果に記録する。合否は、結果の記録と、p95が目標を超えた場合の原因の分析とする(CIのマージ前の必須ゲートにはしない、[assumption])。
  ...
  - 負荷(ログイン以外): リフレッシュ・ログアウト・ロール選択と、認証フィルタを通る要求を、50の同時リクエストで、それぞれ200回以上、繰り返す。計測の対象は、リクエストの受信からレスポンスの送信までである。
  - 負荷(ログイン): 同時のログインの数は、**ハッシュ計算の同時実行の上限(既定はCPUコア数)の2倍**とする(たとえば、4コアで8件)。この条件では、最後の順番待ちの時間が、約0.6秒(2倍の件数 × 0.3秒 ÷ コア数)で、2秒の打ち切りに達しない。計測に用いるアカウントは、計測用に、同時数以上(たとえば50件)を用意し、**正しいパスワードだけ**を使う(誤ったパスワードの繰り返しで、ロックが起こらないようにする)。同じアカウントを、同時に複数のセッションで使わない。200回以上の成功のログインを繰り返す。
  - 集計: p95を、APIごとに算出する。
- **NFR1.1とNFR1.4の関係の解釈**: NFR1.1の「同時50ユーザー」は、50人が、それぞれのAPIを、通常の頻度で呼び出す状態を指す。ログインの同時の集中(ハッシュ計算の同時実行の上限を超える数のログインが、同一の瞬間に集中する場合)は、NFR1.4の既知の限界に含め、NFR1.1のp95の目標の対象外とする。ログインの計測は、上記のとおり、上限の2倍までの同時数で行い、50の同時ログインでは行わない。
...
- 計測: `auth_filter_duration_seconds`(`result`タグ: `hit`・`miss`、observability-design.md NFR5.1)で、フィルタの処理時間を記録する。統合テストで、ヒット時・ミス時の処理時間を計測し、Build and Testの結果に記録する。
...
- `AuthenticationApplicationService`の各メソッドには、`@Transactional`を付けない(外側のトランザクションを作らない)。内側の短いトランザクションは、`AuthenticationApplicationService`だけが、`TransactionTemplate`で、明示的に区切る(reliability-design.md NFR4.1。他の部品は、トランザクションに参加するだけ)。C11の呼び出し元が、トランザクションの中にいることによって、U4の資源の取得順序の不変条件が崩れることを、防ぐ(user-managementのNFR Designの未解消事項の、U5側の対応)。
- **接続を保持しないことの前提**: JPAを使う場合、`spring.jpa.open-in-view=false`を必須とする(`true`のままだと、リクエストの間、EntityManagerが束縛されて、接続が保持されうる。共通基盤への要求、logical-components.mdの追補14番)。テストで、`dummyVerify`の順番待ちの間に、接続プールの使用中の接続が0であること(スタブで待たせて確認)を確認する(logical-components.md NFR8.2)。
