...
## Q2 Follow-up: NFR3（水平スケール要件）からの逸脱の明示的な受入れ（レビュー指摘R-02対応）

Q2で確定したキャッシュ更新方式（起動時全読み込み、書込時にそのインスタンスのキャッシュのみ更新、インスタンス間の無効化通知なし）は、複数インスタンスへ水平拡張した場合、あるインスタンスでの設定変更が他インスタンスへ反映されない。これは`inception/requirements-analysis/requirements.md`のNFR3「同時接続数が想定を超えて拡大しても、線形にリソースを追加することで対応できる設計とする」という要件を実質的に満たさない、という指摘がarchitecture-reviewerのレビューであった。この逸脱を要件レベルの判断として明示的に受け入れるか確認する。

- A. 受入れる。想定利用規模（数十名）と実デプロイが現時点スコープ外（Operationフェーズ未対象）であることを踏まえ、MVPでは単一インスタンス限定を明示的に受け入れる。`traceability.json`のNFR3はGAPとして正直に記録し、将来の水平スケール対応は別途のスコープ判断とする
- B. 今回のBoltでキャッシュ無効化の仕組み（AuditLoggingへの変更イベントを利用したブロードキャスト等）を実装し、NFR3を完全に満たす
- X. Other (please specify)

[Answer]: A. 受入れる。想定利用規模（数十名）と実デプロイが現時点スコープ外（Operationフェーズ未対象）であることを踏まえ、MVPでは単一インスタンス限定を明示的に受け入れる。traceability.jsonのNFR3はGAPとして正直に記録し、将来の水平スケール対応は別途のスコープ判断とする
