...
**上流NFR3からの逸脱の明示的な受入れ**: 上記の制約は、上流`requirements.md`のNFR3（「同時接続数が想定を超えて拡大しても、線形にリソースを追加することで対応できる設計とする」）を完全には満たさない。この逸脱はMVPのスコープ判断として人間により明示的に受け入れられている（`nfr-requirements-questions.md` Q2 Follow-up）。`traceability.json`のNFR3行はこのギャップを`GAP`ステータスとして正直に記録する。
