...
- **責務**: DB接続先設定・メニュー構成・検索条件・一覧表示項目・編集対象外項目・バリデーション・フォーム部品・論理表示名・FK関係/代表表示列という設定全体の保持・CRUD・エクスポート/インポート、静的設定駆動のためのキャッシュ管理 [FR2.1〜FR2.6、FR2.3.1]。管理者専用のCRUD操作群(#15)とは別に、全利用者(非管理者含む)向けにcanList権限でフィルタ済みのメニュー階層を提供する読み取り専用API(#23、frontend-core Unit Functional Designで新設)も持つ [FR3.4]。
- **デプロイ形態**: embedded。
...
- **責務**: 設定管理画面(タブ形式)、メニュー管理画面、スキーマ取り込み画面、権限管理画面群(ロール一覧・編集・割り当て・グループ管理)、アカウント管理画面群(一覧・編集)、監査ログ画面 [FR1.1〜FR1.6、FR2.1〜FR2.6、FR2.3.1、FR5.1〜FR5.6、FR6.4.1〜FR6.4.4、FR7.1〜FR7.5]。複数ロール保有時の作業中ロール「切り替え」自体のUI(Topbarのユーザーメニュー)はU9(frontend-core)の責務であり、本Unitはロール・グループの定義・割り当て設定のみを扱う(frontend-admin Unit Functional Designレビュー R-06フォローで訂正。unit-of-work-story-map.mdのFR5.4行と整合させた)。
- **デプロイ形態**: embedded。
