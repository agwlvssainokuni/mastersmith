...
note: >
  ...
  FKポップアップ検索(BR5.1)の応答についても同様に、代表表示列・recordId生成用の主キー値に
  accessLevel=非表示のカラムが含まれる場合は、その値をそのまま返さない(応答生成前にBR4.2を
  再適用する。R-06フォロー、dynamic-data-access Unit Functional Designレビューiteration 2より)。
  代表表示列自体が非表示の場合は、可視な主キー値(または可視カラム基準集合)を代表表示ラベルとして
  代用する。
```
