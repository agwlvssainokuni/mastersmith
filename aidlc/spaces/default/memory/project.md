...
- サーバー側で算出・付与すべきデータ(列単位の実効READ権限一覧permittedColumnNames、監査ログ用の実行者ユーザーIDactor等)は、クライアント(ブラウザ)向けWEB APIのパラメータにはせず、バックエンドユニット間の内部インタフェース契約にのみ追加する。クライアントが自身の権限範囲や実行者IDを指定できる余地を作らないため。 (learned 2026-09-14) <!-- cid:260910-master-mgmt-app:contract-design:5f10a401f2c2589580c597d5e45a359c91fd7cdd01831c1125d973f01b5a12e8 -->
